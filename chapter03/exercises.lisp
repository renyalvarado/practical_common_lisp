...
(add-record (make-cd "Roses" "Katy Mattea" 7 t))
(add-record (make-cd "Fly" "Dixie Chicks" 8 t))
(add-record (make-cd "Home" "Dixie Chicks" 9 t))

(defun artist-selector (artist)
  #'(lambda (cd) (equal (getf cd :artist)
			artist)))

(defun title-selector (title)
  #'(lambda (cd) (equal (getf cd :title)
			title)))

(defun rating-selector (rating)
  #'(lambda (cd) (equal (getf cd :rating)
			rating)))

(defun ripped-selector (ripped)
  #'(lambda (cd) (equal (getf cd :ripped)
			ripped)))
...
(defun make-comparisons-list (fields)
  (do* ((expresions fields)
	(result nil))
       ((null expresions) result)
    (push (make-comparison-expr (pop expresions)
				(pop expresions))
	  result)))
